...
def trans_predict(model,inp_seq,opt,movie_to_id,id_to_movie):

    seq = np.zeros([opt['max_seq_len']], dtype=np.int32)
    idx = opt['max_seq_len'] - 1
    for i in reversed(inp_seq):
        seq[idx] = i
        idx -= 1
        if idx == -1: break
    
    all_items = list(movie_to_id.values())

    user_rep = model.get_user_rep(torch.tensor([1]),torch.from_numpy(seq.reshape(1,-1)).type(torch.LongTensor))
    predictions = -model.predict(user_rep,torch.tensor(all_items))

    # predictions = predictions.detach().cpu()
    predictions = predictions.detach().cpu()[0]
    # print(sorted(predictions,reverse=True))
    top_10 = predictions.argsort().tolist()[:10]

    return list(map(lambda x:id_to_movie[x],top_10))
    # print(list(map(lambda x:id_to_movie[x],top_10)))
    # print(predictions.argsort().tolist())
    # print(predictions.argsort().argsort())
    # rank = predictions.argsort().argsort()

def rnn_predict(model,inp_seq,opt,movie_to_id,id_to_movie):
    seq = np.zeros([opt['max_seq_len']], dtype=np.int32)
    seq_len = min(len(inp_seq) + 1,opt['max_seq_len'])
    idx = seq_len - 1
    ...
    user_rep = model.get_user_rep(torch.tensor([1]),torch.from_numpy(seq.reshape(1,-1)).type(torch.LongTensor),torch.tensor([seq_len]))
    predictions = -model.predict(user_rep,torch.tensor(all_items))

    # predictions = predictions.detach().cpu()
    predictions = predictions.detach().cpu()[0]

    # print(sorted(predictions,reverse=True)[:10])
    top_10 = predictions.argsort().tolist()[:10]
    # print(top_10)
    id_to_movie[0]="<PAD>"
    return list(map(lambda x:id_to_movie[x],top_10))  
